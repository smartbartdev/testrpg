...
function Clipboard({apiPath}: {apiPath: string}) {
    ...
    return (
        <div className={"flex items-center space-x-2 transition-all"}>
            <button data-clicked={clicked} onClick={setClipBoard} className={"data-[clicked='true']:text-blue-600"}>Click here to copy <span className={"font-mono font-bold text-gray-600"}>curl</span> command</button>
        ...
        <Card className={"text-left"} data-testid={"api-explainer"}>
            ...
            <CardContent>
                ...
                <p>This parameter has the following valid values: <span
                    className={"font-mono text-sm font-bold text-gray-600"}>thief, knight, mage, brigadier</span></p>
            </CardContent>
